#!/bin/bash
# Make it as painless as possible to conduct a 1000 user controlled test lasting
# about 1 hour. The scenario is called, professional1000.
#
# Run this and put it in the background with the output re-directed to a file.
# e.g.
#     /home/bench/fluline/autoprofrun.sh >/home/bench/fluline/autoprofrun.log 2>&1 &
#
# Overview
# ========
# 1. Clear out the work area, removing echo, log, dump and results files
# 2. Remove the existing 'scenario execution' professional1000.
# 3. Re-extract or reset the data that will be re-used
#    -  MALE and FEMALE
#    -  Combine them
# 4. Create a new 'scenario execution' for professional1000, and assign it to
#    172.23.23.66, 172.23.23.67, 172.23.23.68, 172.23.23.69.
# 5. Distribute the new scenario.
# 6. Execute the scenario.  135.2.78). Once this has started, ps should show:
...
#         /home/bench/fluline/work/professional1000.1/timout.html
...
for i in 66 67 68 69
do
minitest 172.23.23.$i $E2_HOME_PORT EXEC "cp patch_4.sh patch_nlb.sh" </dev/null
done
# *****************************************************************************
# Re-extract the data that needs to be refreshed.
# 
cd $PATH_HOME/data
head -1 MALE.db > FEMALE.db
tidydata.sh 15500 MALE FEMALE
$PATH_AWK 'NR > 1 { print $0}' FEMALE.db >> MALE.db
head -1 MALE.db > FEMALE.db
cd $PATH_HOME
#
# *****************************************************************************
