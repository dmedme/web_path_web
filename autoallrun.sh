#!/bin/bash
# Make it as painless as possible to conduct a 3200 public + 1000 professionl user
# controlled test lasting about 1 hour. The scenario is called, 3200pub1000prof.
#
...
# 2. Remove the existing 'scenario execution' 3200pub1000prof.
# 3. Re-extract or reset the data that will be re-used
#    -  MALE and FEMALE
#    -  Combine them
# 4. Create a new 'scenario execution' for 3200pub1000prof, and assign it to
...
#         /home/bench/fluline/work/3200pub1000prof.1/timout.html
...
# 
cd $PATH_HOME/data
tidydata.sh 100000 MALE FEMALE
tail -45000 FEMALE.db >> MIXED.db
tail -45000 MALE.db >> MIXED.db
head -55000 FEMALE.db >FEMALE.tmp
mv FEMALE.tmp FEMALE.db
head -55000 MALE.db >MALE.tmp
mv MALE.tmp MALE.db
cd $PATH_HOME
#
# *****************************************************************************
