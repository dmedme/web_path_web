...
if [ ! "$PATH_OS" = NT4 ]
then
    mkfifo $PATH_HOME/sessions/script_out_fifo.$E2_WEB_PID
    until [ -p $PATH_HOME/sessions/web_fifo.$E2_WEB_PID -a -p $PATH_HOME/sessions/script_in_fifo.$E2_WEB_PID ]
    do
        sleep 1
    done
fi
...
while :
do
    e2fifin ${NAMED_PIPE_PREFIX}script_out_fifo.$E2_WEB_PID >$PATH_HOME/sessions/tmp$E2_WEB_PID.html
    echo $PATH_HOME/sessions/tmp$E2_WEB_PID.html
#
# Wait for the previous incarnation to be displayed
#
    while [ -f $PATH_HOME/sessions/tmp$E2_WEB_PID.html ]
    do
        sleep 1
    done
...
sleep 5
rm -f $PATH_HOME/sessions/tmp$E2_WEB_PID.html
if [ ! "$PATH_OS" = NT4 ]
then
