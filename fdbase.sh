...
PATH_HOME=${PATH_HOME:-/home/bench/testbed}
# O/S type for establishing executables directory (PATH_SOURCE) and text editor
# (PATH_EDITOR)
PATH_OS=${PATH_OS:-LINUX}
PATH_SOURCE=${PATH_SOURCE:-/home/bench/testbed/web_path_web}
...
*)
    ...
    PATH=$PATH_HOME/lin${arch}bin:$PATH_SOURCE:$PATH
    export PATH
    ;;
esac
fi
...
elif [ "$PATH_DRIVER" = recordx ]
then
export FDSCRIPT=fdscript_xgui.sh
else
#export FDSCRIPT=fdscript_capweb.sh
