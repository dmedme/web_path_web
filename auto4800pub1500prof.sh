#!/bin/bash
# Make it as painless as possible to conduct a 4800 public + 1500 professionl user
# controlled test lasting about 1 hour. The scenario is called, 4800pub1500prof.
#
...
# 2. Remove the existing 'scenario execution' 4800pub1500prof.
# 3. Re-extract or reset the data that will be re-used
#    -  MALE and FEMALE
#    -  Combine them
# 4. Create a new 'scenario execution' for 4800pub1500prof, and assign it to
...
#         /home/bench/fluline/work/4800pub1500prof.1/timout.html
...
for i in 66 67 68 69
do
minitest 172.23.23.$i $E2_HOME_PORT EXEC "cp patch_12.sh patch_nlb.sh" </dev/null
done
# *****************************************************************************
...
tail -67500 FEMALE.db >> MIXED.db
tail -67500 MALE.db >> MIXED.db
head -82500 FEMALE.db >FEMALE.tmp
mv FEMALE.tmp FEMALE.db
head -82500 MALE.db >MALE.tmp
mv MALE.tmp MALE.db
cd $PATH_HOME
#
# *****************************************************************************
